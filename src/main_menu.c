...
#define tCherenSpriteId data[12]
#define tBiancaSpriteId data[13]
...
static void Task_NewGameBirchSpeech_LoadCheren(u8 taskId)
{
    ...
    if (!RunTextPrintersAndIsPrinter0Active() && gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        spriteId = gTasks[taskId].tCherenSpriteId;
...
static void Task_NewGameBirchSpeech_LoadBianca(u8 taskId)
{
    ...
    if (!RunTextPrintersAndIsPrinter0Active() && gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        spriteId = gTasks[taskId].tBiancaSpriteId;
        gSprites[spriteId].x = 180;
        gSprites[spriteId].y = 60;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_ShrinkPlayer(u8 taskId)
{
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tCherenSpriteId].invisible = TRUE;
            gSprites[gTasks[taskId].tBiancaSpriteId].invisible = TRUE;
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    hilbertSpriteId = CreateTrainerSprite(FacilityClassToPicIndex(FACILITY_CLASS_HILBERT), 120, 60, 0, &gDecompressionBuffer[0]);
    ...
    gTasks[taskId].tBiancaSpriteId = biancaSpriteId;
}
...
#undef tMaySpriteId
#undef tCherenSpriteId
#undef tBiancaSpriteId
...
static void MainMenu_FormatSavegamePokedex(void)
{
    ...
    if (FlagGet(FLAG_SYS_POKEDEX_GET) == TRUE)
    {
        if (IsNationalPokedexEnabled())
            ...
            dexCount = GetUnovaPokedexCount(FLAG_GET_CAUGHT);
